...
public:
	...
	* 
	...
	void save_alumni_list(const string& fileName, const alumni_list& alumniList) {// 保存校友列表到文件 
		if (alumniList.empty()) {
			return; // 如果列表为空，直接返回
		}
	...
	alumni_list load_alumni_list(const string& fileName) {
	...
	vector<Department> load_department_list(const string& fileName) {
